...
class FrontPadLogicFactory
{
    ...
    public function newOrder(array $data)
    {
        ...
        $result = Http::asForm()->post(config("frontpad.api_url")."?new_order", [
            ...
            'pay' => $data["pay"] ?? 1, //отметка оплаты заказа, значение можно посмотреть в справочнике “Варианты оплаты”;
            ...
            'hook_status' => $data["hook_status"] ?? [1], //массив статусов заказов, значение кодов API можно посмотреть в справочнике программы.Передается в формате аналогичном массиву товаров (не более 5), см. пример;
